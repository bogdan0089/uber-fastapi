...
async def get_my_trips(user: CurrentUser, limit: int = 10, offset: int = 0) -> list[ResponseTrip]:
    return await TripService.get_my_trips(user.id, limit, offset)

@router_trip.get("/{trip_id}", response_model=ResponseTrip)
async def get_trip(trip_id: int, _: CurrentUser) -> Trip:
...
async def cancel_trip(trip_id: int, user: CurrentUser) -> ResponseTrip:
